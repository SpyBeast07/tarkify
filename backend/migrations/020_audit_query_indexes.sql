...
CREATE INDEX IF NOT EXISTS idx_audit_logs_module ON audit_logs ((
  CASE
    ...
    WHEN event LIKE 'general%' OR event LIKE 'brand%' OR event LIKE 'oauth%' OR event LIKE 'security%' OR event LIKE 'storage%' OR event LIKE 'features%' OR event LIKE 'notifications%' OR event LIKE 'seo%' OR event LIKE 'legal%' OR event = 'settings_viewed' THEN 'Settings'
