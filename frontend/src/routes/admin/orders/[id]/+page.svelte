<script lang="ts">
	...
	import {
		...
		CreditCard, FileText, Download, History, Info
	} from '@lucide/svelte';
...
<AdminPage {loading} {error} onRetry={loadOrder}>
	{#if order}
		<AdminPageHeader title={`Order #${order.id.substring(0, 8)}`} description={`Placed on ${formatDate(order.created_at)}`}>
			<Button variant="ghost" href="/admin/orders">
				<ArrowLeft size={16} />
				Back to Orders
			</Button>
			<Button variant="ghost" href={`/admin/payments/${order.id}`}>
				<CreditCard size={16} />
				View Payment
			</Button>
		</AdminPageHeader>
		...
		{#if activeTab === 'overview'}
			<div class="detail-grid">
				<div class="detail-column detail-column-left">
					<SectionCard title="Order Information" icon={ShoppingCart}>
				...
				<div class="detail-column detail-column-right">
					<SectionCard title="Download Tokens" icon={Download}>
					...
					<SectionCard title="Entitlements" icon={Info}>
					...
					<SectionCard title="Emails Sent" icon={FileText}>
					...
					<div class="section-card-placeholder">
						<SectionCard title="Internal Notes" icon={Info}>
