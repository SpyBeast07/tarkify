...
<svelte:head>
	<title>System Health | Tarkify Admin</title>
</svelte:head>
	...
	<AdminPageHeader title="System Health" description="Operational monitoring across application, database, storage, and connected services">
		{#if data}
			<div class="header-status">
				...
				{/if}
				<Button variant="ghost" size="sm" onclick={() => load()} disabled={loading || refreshing} aria-label="Refresh system health">
	...
	<AdminPage {loading} {error} onRetry={() => load()}>
		{#if data}
			<AdminStack gap="md">
				...
				<AdminGrid cols={{ default: 1, sm: 2, md: 3 }} gap="md">
