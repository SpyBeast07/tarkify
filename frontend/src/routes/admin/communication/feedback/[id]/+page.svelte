...
{#snippet overviewContent(record: any)}
	<div class="ov">
		<div class="ov-item"><span class="ov-label">Name</span><span class="ov-value">{record.name || 'Anonymous'}</span></div>
		<div class="ov-item"><span class="ov-label">Email</span><span class="ov-value mono">{record.email || '—'}</span></div>
		<div class="ov-item"><span class="ov-label">Product</span><span class="ov-value">{record.product}</span></div>
		<div class="ov-item"><span class="ov-label">Rating</span><span class="ov-value">{'★'.repeat(record.rating)}{'☆'.repeat(5 - record.rating)} ({record.rating}/5)</span></div>
...
<CommunicationDetailView
	...
	{recordId}
	title="Feedback"
	backHref="/admin/communication/feedback"
	canReply={true}
	recipientEmail={(r) => r.email || ''}
	defaultSubject={(r) => `Re: Feedback about ${r.product}`}
