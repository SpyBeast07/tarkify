...
<AdminPage {loading} {error} onRetry={load}>
	...
	<div class="toolbar">
		<div class="search-bar">
			<span class="search-icon"><Search size={16} aria-hidden="true" /></span>
			<input
				type="text"
		...
		<div class="pagination">
			...
			<div class="pagination-buttons">
				<Button
					...
					onclick={() => goToPage((params.page ?? 1) - 1)}>Previous</Button
				>
				<Button
					variant="ghost"
					size="sm"
					disabled={(params.page ?? 1) >= totalPages}
					onclick={() => goToPage((params.page ?? 1) + 1)}>Next</Button
				>
			</div>
		</div>
	{/if}
