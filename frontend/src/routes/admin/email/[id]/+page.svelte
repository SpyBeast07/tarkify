<script lang="ts">
	...
	import { ArrowLeft, RotateCcw, AlertTriangle, Mail, Code } from '@lucide/svelte';
...
<AdminPage {loading} {error} onRetry={load}>
	{#if email}
		<AdminPageHeader title="Email" description={subjectOf()}>
			<Button variant="ghost" href="/admin/email">
				<ArrowLeft size={16} />
				Back
			</Button>
			{#if email.status === 'failed'}
				<Button variant="ghost" onclick={() => (confirmResend = true)}>
					<RotateCcw size={16} />
					Resend
				</Button>
			{/if}
		...
		{#if activeTab === 'overview'}
			<div class="detail-grid">
				...
				<div class="detail-column-right">
					<AdminSection title="Delivery">
						<div class="detail-list">
							<div class="detail-item"><span class="detail-label">Status</span><span class="detail-value"><EmailStatusBadge status={email.status} /></span></div>
							{#if email.error}
		...
		{#if confirmResend}
			<div class="confirm-dialog" role="alertdialog" aria-labelledby="confirm-title">
				<div class="confirm-content">
					...
					<h4 id="confirm-title">Confirm resend</h4>
					<p>This will re-send the email to <strong>{email.recipient}</strong> using the configured provider.</p>
					<div class="confirm-buttons">
						<Button variant="primary" size="sm" disabled={actionLoading} onclick={handleResend}>
							{actionLoading ? 'Sending...' : 'Confirm Resend'}
						</Button>
...
<style>
	...
	.alert-success { background: rgba(39, 59, 9, 0.12); color: #5a7a1a; border: 1px solid rgba(39, 59, 9, 0.2); }
	.alert-error { background: rgba(220, 38, 38, 0.1); color: #ef4444; border: 1px solid rgba(220, 38, 38, 0.2); }

	.tab-bar {
		display: flex; gap: 0; margin-bottom: 1.5rem;
		border-bottom: 1px solid var(--color-glass-border); overflow-x: auto;
	}
	.tab {
		padding: 0.625rem 1.25rem; font-size: 0.875rem; font-weight: 600;
		background: none; border: none; border-bottom: 2px solid transparent;
		cursor: pointer; opacity: 0.55; color: var(--color-text); transition: all 0.15s ease; white-space: nowrap;
	}
	...
	.detail-column-left, .detail-column-right { display: flex; flex-direction: column; gap: 1.5rem; }
	.detail-list { display: flex; flex-direction: column; gap: 0.75rem; margin-top: 0.5rem; }
	.detail-item { display: flex; flex-direction: column; gap: 0.125rem; }
	.detail-label {
		font-size: 0.8rem; opacity: 0.5; font-weight: 500;
		text-transform: uppercase; letter-spacing: 0.04em;
	}
	.detail-value { font-size: 0.95rem; line-height: 1.5; word-break: break-word; }
	.detail-value.mono { font-family: var(--font-accent); font-size: 0.85rem; opacity: 0.75; }
	.detail-value.error-text { color: #ef4444; }
	.actions-grid { display: flex; flex-wrap: wrap; gap: 0.5rem; margin-top: 1rem; }

	.mono-small { font-family: var(--font-accent); font-size: 0.8rem; opacity: 0.7; }

	.preview-tabs { display: flex; gap: 0.5rem; margin-bottom: 0.5rem; }
	.preview-label { display: inline-flex; align-items: center; gap: 0.3rem; font-size: 0.8rem; opacity: 0.6; }
	.html-preview {
		width: 100%; min-height: 420px; border: 1px solid var(--color-glass-border);
		border-radius: 12px; background: #fff;
	}
	.text-preview {
		margin-top: 1rem; padding: 1rem; background: rgba(0,0,0,0.15);
		border-radius: 10px; font-size: 0.8rem; white-space: pre-wrap; overflow-x: auto;
	}
	...
	.confirm-icon { display: flex; justify-content: center; margin-bottom: 0.75rem; opacity: 0.6; }
	.confirm-content h4 { font-size: 1rem; font-weight: 700; margin-bottom: 0.5rem; }
	.confirm-content p { font-size: 0.85rem; opacity: 0.7; margin-bottom: 1rem; }
